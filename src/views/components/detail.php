<div class="flex flex-col overflow-hidden rounded-lg">
    <div class="">
        <img
            src="img/menu/<?= htmlspecialchars($detailMenu['image']) ?>"
            alt="Mie Ayam"
            class="w-sm aspect-square object-center object-cover hover:scale-105 transition-all duration-300 ease-in-out" />
    </div>
    <form hx-post="/" method="post" hx-target="#cookie" class="text-base font-semibold px-4 py-2.5">
        ...
        <input type="hidden" name="idProduct" value="<?= htmlspecialchars($detailMenu['id']) ?>">
        <input type="hidden" name="idTable" value="<?= $param ?>">
